...
void compute_all_powers_vec_plain(const vector<double> &my_vec, int degree, vector<vector<double>> &powers)
{
    ...
    for (int i = 3; i <= degree; i++)
    {
        ...
        if(i % 2 == 0) {
            powers[i][0] = powers[cand][0] * powers[i - cand][0];
        } else {
            for(int j = 0; j < my_vec.size(); j++) {
                powers[i][j] = powers[cand][j] * powers[i -cand][0];
            }
        }
    }
...
// Helper for Tree method, computes powers of x in a tree
void compute_all_powers_vec_cipher(const Ciphertext &ctx, int cols, int degree, Evaluator &evaluator, GaloisKeys &gal_keys, RelinKeys &relin_keys, vector<Ciphertext> &powers)
{
    ...
    // powers.resize(degree + 1);
    // powers[1] = ctx;
    // powers[2] = cipher_dot_product(ctx, ctx, cols, relin_keys, gal_keys, evaluator);
